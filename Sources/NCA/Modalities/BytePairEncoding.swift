...
extension BytePairEncoder {
  ...
  internal func splitRecursively(_ token: String) -> [String] {
    ...
    let rightParts = vocabulary.contains(pair.right) ? [pair.right] : splitRecursively(pair.right)
  ...
  internal static func splitWithDelimiters(
    ...
    let matches = glossaryRegex.matches(in: token, range: NSRange(token.startIndex..., in: token))
    var parts = [String]()
