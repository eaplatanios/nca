...
extension QNLI {
  public init<A: Architecture>(
    ...
    self.directoryURL = taskDirectoryURL.appendingPathComponent("QNLI")

    let dataURL = directoryURL.appendingPathComponent("data")
    let compressedDataURL = dataURL.appendingPathComponent("downloaded-data.zip")

    // Download the data, if necessary.
    try maybeDownload(from: QNLI.url, to: compressedDataURL)
    ...
    let dataFilesURL = extractedDirectoryURL.appendingPathComponent("QNLI")
    self.trainExamples = try QNLI.load(
      fromFile: dataFilesURL.appendingPathComponent("train.tsv"),
      fileType: .train)
    self.devExamples = try QNLI.load(
      fromFile: dataFilesURL.appendingPathComponent("dev.tsv"),
      fileType: .dev)
    self.testExamples = try QNLI.load(
    ...
    let exampleMapFn: (Example) -> DataBatch = { example -> DataBatch in
      ...
      return DataBatch(inputs: textBatch, labels: example.entailment.map { Tensor($0 ? 1 : 0) })
    }
...
extension QNLI {
  ...
  /// URL pointing to the downloadable ZIP file that contains the QNLI dataset.
