...
public struct MultiHeadAttention: Layer, Regularizable { // <Scalar: TensorFlowFloatingPoint>: Layer {
  ...
  public func callAsFunction(_ input: AttentionInput<Scalar>) -> Tensor<Scalar> {
    ...
    // Since the attention mask is set to 1.0 for positions we want to attend to and 0.0 for masked
    // positions, we create a tensor which is 0.0 for positions we want to attend to and -10000.0
    // for masked positions. Since we are adding this tensor to the raw scores before the softmax,
    // this is effectively the same as removing the masked entries entirely.
    let attentionMask = input.mask.expandingShape(at: 1) // [B, 1, F, T]
    attentionScores = attentionScores - 10000 * (1 - attentionMask)

    // Normalize the attention scores to convert them to probabilities. We are also dropping out
    // entire tokens to attend to, which might seem a bit unusual, but it is taken from the
