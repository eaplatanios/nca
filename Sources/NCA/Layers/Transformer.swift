...
public struct TransformerEncoder: Layer, Regularizable { // <Scalar: TensorFlowFloatingPoint>: Layer {
  ...
  public func callAsFunction(_ input: TransformerInput<Scalar>) -> Tensor<Scalar> {
    // The transformer performs sum residuals on all layers and so the input needs to have the same
    // depth as hidden size of the transformer.
    precondition(
...
public struct TransformerEncoderLayer: Layer, Regularizable { // <Scalar: TensorFlowFloatingPoint>: Layer {
  ...
  public func callAsFunction(_ input: TransformerInput<Scalar>) -> Tensor<Scalar> {
    ...
    attentionOutput = attentionLayerNormalization(attentionOutput + input.sequence)
